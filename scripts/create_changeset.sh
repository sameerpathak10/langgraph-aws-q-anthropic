...
usage() {
  cat <<EOF
Usage: $0 STACK_NAME TEMPLATE_FILE LAMBDA_S3_BUCKET LAMBDA_S3_KEY [CHANGESET_PREFIX]
Example:
...
aws cloudformation create-change-set \
  ...
  --parameters ParameterKey=LambdaS3Bucket,ParameterValue="$LAMBDA_S3_BUCKET" ParameterKey=LambdaS3Key,ParameterValue="$LAMBDA_S3_KEY"
